...
if ( ! class_exists( 'Astra_Sites_Compatibility_Elementor' ) ) :
	...
	class Astra_Sites_Compatibility_Elementor {
		...
		public function register_page_typography( Controls_Stack $element, $section_id ) {
			...
			if (
				in_array( Schemes\Color::get_type(), Schemes\Manager::get_enabled_schemes(), true ) ||
				in_array( Schemes\Typography::get_type(), Schemes\Manager::get_enabled_schemes(), true )
			) {
				$element->add_control(
					'global_warning',
					array(
						...
						'raw'             => sprintf( __( 'We have noticed that your Elementor Default Colors and Fonts settings are enabled. To see Starter Template settings in action you need to turn off the Global Settings from %1$s.', 'astra-sites' ), '<a href="' . admin_url( 'admin.php?page=elementor' ) . '" target="_blank">here</a>' ),
				...
				$element->add_control(
					...
					array(
						...
						'selectors' => array(
							'.elementor-button-wrapper .elementor-button,.elementor-button-wrapper .elementor-button:visited,.wp-block-button .wp-block-button__link, .menu-toggle,button,.ast-button,.button,input#submit,input[type="button"],input[type="submit"],input[type="reset"]' => 'border-color: {{VALUE}}; background: {{VALUE}};',
				...
				$element->add_control(
					...
					array(
						...
						'type'      => Controls_Manager::COLOR,
						'selectors' => array(
							'body,h1,.entry-title a,.entry-content h1,h2,.entry-content h2,h3,.entry-content h3,h4,.entry-content h4,h5,.entry-content h5,h6,.entry-content h6,.wc-block-grid__product-title, .main-header-menu a, .widget-title' => 'color: {{VALUE}};',
						),
						'condition' => array(
							'astra_sites_page_setting_enable' => 'yes',
						),
				...
				$element->add_control(
					'astra_sites_heading_base_color',
					array(
						'label'     => __( 'Heading Base Color (H1-H6)', 'astra-sites' ),
						'type'      => Controls_Manager::COLOR,
						'selectors' => array(
							'h1, .entry-content h1, h2, .entry-content h2, h3, .entry-content h3, h4, .entry-content h4, h5, .entry-content h5, h6, .entry-content h6' => 'color: {{VALUE}};',
						),
						'condition' => array(
							'astra_sites_page_setting_enable' => 'yes',
						),
